...
//		printf("input signal: (%f, %f)\n", x, y);
	...
	chains.first.anchor_mut() = Spinner::fromCartesian(winWidth/2, 100);
	chains.second.anchor_mut() = Spinner::fromCartesian(100, winHeight/2);
		...
		while(SDL_PollEvent(&e)){
			...
			case SDL_KEYUP:
				...
				case SDLK_SPACE:
					...
					chains = createDFT(drawing.data_vec(), DFT_DEPTH);
					break;
				};
				break;
...
//				printf("Recv evt! : %d\n", e.type);
		...
		offset = chains.second.anchor_ref(); // move draw point to that of the anchor
		for(int i = 0; i < chains.second.chain_length(); i++){
			//draw spinner circles]
			SDL_SetRenderDrawColor(renderer, 0x00, 0x00, 0x00, 0x00);
			DrawCircle(renderer, offset.getX(), offset.getY(), chains.second.spinners_ptr()[i].rho);
			// draw spinner vector lines (relative to previous vector -- relative to the "offset" spinner)
			SDL_SetRenderDrawColor(renderer, 0xFF, 0x00, 0x00, 0xFF);
			SDL_RenderDrawLine(renderer,
					(int)(offset.getX()),
					(int)(offset.getY()),
					(int)(offset.getX() + chains.second.spinners_ptr()[i].getX()),
					(int)(offset.getY() + chains.second.spinners_ptr()[i].getY())
			);
			offset = offset+chains.second.spinners_ptr()[i];
		}
		tracePoint.y = offset.getY();
		antiTracePoint.x = offset.getX();
		offset = chains.first.anchor_ref();

		for(int i = 0; i < chains.first.chain_length(); i++){
			//draw spinner circles
			SDL_SetRenderDrawColor(renderer, 0x00, 0x00, 0x00, 0xFF);
			DrawCircle(renderer, offset.getX(), offset.getY(), chains.first.spinners_ptr()[i].rho);
			// draw spinner vector lines (relative to previous vector -- relative to the "offset" spinner)
			SDL_SetRenderDrawColor(renderer, 0xFF, 0x00, 0x00, 0xFF);
			SDL_RenderDrawLine(renderer,
					(int)(offset.getX()),
					(int)(offset.getY()),
					(int)(offset.getX() + chains.first.spinners_ptr()[i].getX()),
					(int)(offset.getY() + chains.first.spinners_ptr()[i].getY())
			);
			offset = offset+chains.first.spinners_ptr()[i];
		}
		...
		for(int i = 0; i < chains.second.chain_length(); i++){
			chains.second.spinners_mut()[i].theta +=  chains.second.spinners_ptr()[i].freq * 2*M_PI / sigLen;
		}
		for(int i = 0; i < chains.first.chain_length(); i++){
			chains.first.spinners_mut()[i].theta +=  chains.first.spinners_ptr()[i].freq * 2*M_PI / sigLen;
		}
		SDL_Delay(16);
	}
	close();
